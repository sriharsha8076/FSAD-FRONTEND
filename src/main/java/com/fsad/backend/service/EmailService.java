        ...
        for (int attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                if (attempt == maxRetries) {
                    throw new RuntimeException("Failed to send OTP email after " + maxRetries + " attempts: " + e.getMessage(), e);
                }
                try {
                    // Wait 2 seconds before retrying to bypass Gmail burst speed filters
                    Thread.sleep(2000);
